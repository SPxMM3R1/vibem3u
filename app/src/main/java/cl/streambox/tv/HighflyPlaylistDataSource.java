...
/** Decodes papacito.cfd playlists for Media3, never media segments. */
@UnstableApi
final class HighflyPlaylistDataSource implements DataSource {
    ...
    HighflyPlaylistDataSource(DataSource upstream) {
    ...
    public long open(DataSpec dataSpec) throws IOException {
        ...
        if (!isPapacitoPlaylist(dataSpec == null ? null : dataSpec.uri)) {
        ...
        while ((count = upstream.read(buffer, 0, buffer.length)) != C.RESULT_END_OF_INPUT) {
            if (Thread.currentThread().isInterrupted()) {
                throw new IOException("Solicitud Highfly cancelada.");
            }
            if (count <= 0) continue;
            total += count;
            if (total > HighflyHlsDecoder.MAX_PLAYLIST_BYTES) {
                throw new IOException("Playlist Highfly demasiado grande.");
            }
            output.write(buffer, 0, count);
        }
        playlistBody = HighflyHlsDecoder.decodeIfNeeded(output.toByteArray());
    ...
    private static boolean isPapacitoPlaylist(Uri uri) {
        return uri != null
                && "papacito.cfd".equalsIgnoreCase(uri.getHost())
                && uri.getPath() != null
    ...
    static final class Factory implements DataSource.Factory {
        ...
        public DataSource createDataSource() {
            return new HighflyPlaylistDataSource(upstreamFactory.createDataSource());
        }
    }
}
